...
let historialLaPolla = new Set();

function laPolla(num) {
  let frase;

  num = parseInt(num);

  if (isNaN(num) || num < 0 || num > lapolla.length) {
    document.getElementById("la-polla-resultado").innerText =
      "Número inválido. Ingresa entre 0 y " + lapolla.length + " (0 = aleatorio)";
    return;
  }

  if (num === 0) {
    const indice = Math.floor(Math.random() * lapolla.length);
    frase = lapolla[indice];
    num = indice + 1;
  } else {
    frase = lapolla[num - 1];
  }

  if (historialLaPolla.has(num)) {
    frase = "[REPETIDA] " + frase;
  } else {
    historialLaPolla.add(num);
  }
